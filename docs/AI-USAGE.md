...
- **Was gut lief**:
  ...
  - **Two-Stage-Review fängt 4 echte Bugs**: Gleiche Reviewer-Disziplin wie Single-Memo — Spec-Reviewer + Code-Quality-Reviewer in sequenziellen Subagent-Dispatches haben vier produktionskritische Fehler gefunden (Details in §11.1 der Spec + "Was nicht klappte").
...
- **Lektion (für die 40%-Achse)**:
  **Vier Review-gefundene Bugs in 12 Build-Steps — das ist das erwartete Verhältnis bei Subagent-Dispatch.** Subagents implementieren korrekt innerhalb ihres Scopes, aber sie sehen keine DB-Constraints aus anderen Migrations (FK-Violation-Bug), keine PostgreSQL-spezifischen Syntax-Regeln (server_default-Bug), und keine Error-Handling-Gaps die erst unter Production-Load sichtbar werden (RateLimitError-Bug). Two-Stage-Review ist die einzige Schicht die das fängt. Heuristik: jeder Build-Step mit DB-Schema oder Error-Handling bekommt einen Reviewer der explizit gegen die echte DB und gegen den Spec-Error-Handling-Abschnitt checkt — nicht nur gegen den Code.
...
- **Offenes Issue**: `BudgetCapExceeded`-Global-Handler gibt 503 zurück, aber Spec §6 sagt 402 für `/memos/batch`. Per-route `HTTPException(402)` macht den `/batch`-Endpoint korrekt, aber der globale Handler bleibt inkonsistent. Folge-Issue zur Konsolidierung — beide Memo-Endpoints sollten 402 returnen.
