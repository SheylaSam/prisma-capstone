...
## 2026-05-14 · Issue #46 abschliessen — fehlende Spec-Tests nachreichen (PR #100)
- **Agent**: Claude Code (Sonnet 4.6), Main-Context, mit `superpowers:writing-plans` (Plan-Mode) + `Explore`-Subagent für Codebase-Zustand-Audit.
- **Scope**: Issue #46 ("RankingRunService auf alle 5 Modelle erweitern") war noch offen, obwohl PR #71 alle 4 Acceptance Criteria bereits implementiert hatte. Die Session identifiziert den Gap, ergänzt zwei fehlende Tests aus der Spec und schliesst das Issue via PR #100.
- **Was gut lief**:
  - **Spec-gegen-Tests-Diff als Gap-Finder**: Bevor irgendein Code geschrieben wurde, haben wir die Spec (`docs/specs/2026-05-09-ranking-service-multi-model.md`) gegen die vorhandenen Test-Files abgeglichen. Die Spec-Tabelle listete `test_sweet_spot_triggers_with_five_models` explizit auf — der Test fehlte komplett in `test_ranking_run_service_multi_model.py`. Ohne diesen Vergleich hätte man das Issue einfach kommentarlos geschlossen.
  - **Issue-State ≠ Code-State erkennen**: PR #71 hatte alle ACs implementiert, ohne `Closes #46` in der Beschreibung. Wer nur das Issue-Label ("blocked") liest, denkt die Arbeit ist offen. Wer die tatsächlichen Files liest, sieht: `ranking_run_service.py:73-79` ruft bereits alle 5 Modelle auf. **Lehre**: vor einer Re-Implementation immer erst `git log --follow <file>` + Datei-Inhalt prüfen. Spart Doppelarbeit.
  - **Deterministisch-but-differenziert**: `test_sweet_spot_triggers_with_five_models` verlässt sich auf den `StubMarketDataProvider` mit `zlib.crc32`-Seed pro Ticker — verschiedene Tickers haben verschiedene Random-Walks → echte Rang-Varianz zwischen Modellen → Sweet-Spot-Trigger möglich. Der Test ist prozess-stabil (kein Python-`hash()`), run-stabil (fester Seed) und diskriminiert echte Sweet-Spot-Logik von Fallback-Verhalten.
- **Was nicht klappte**: Nichts Wesentliches. Die Scope-Begrenzung (nur Tests, kein Produktionscode) hat Drift verhindert.
- **Methodisches Mini-Learning**: **Spec-Tabellen als Checklisten nehmen, nicht als Prosa lesen.** Die Spec hatte eine Test-Tabelle mit 3 Zeilen — wir haben alle 3 gegen die existierenden Test-Funktionsnamen gecheckt. Zwei waren da, einer nicht. Das ist eine 2-Minuten-Aufgabe mit `grep`, aber sie liefert exakt den Scope für die Session. Wer die Spec nur überfliegt, übersieht, dass `test_sweet_spot_triggers_with_five_models` als explizite Zeile steht, nicht als nice-to-have.
- **Token-Kosten**: ~15k Tokens Sonnet 4.6 (1 Explore-Subagent + Plan-Mode + 2 File-Edits + Tests + PR); ~0.10 USD.
