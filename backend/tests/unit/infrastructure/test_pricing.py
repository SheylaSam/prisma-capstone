...
class TestPricingRegistry:
    ...
    def test_no_float_values_anywhere(self) -> None:
        ...
        for model_name, pricing in PRICING.items():
            assert isinstance(pricing.input_per_mtok, Decimal), model_name
            assert isinstance(pricing.output_per_mtok, Decimal), model_name
            if pricing.embed_per_mtok is not None:
                assert isinstance(pricing.embed_per_mtok, Decimal), model_name
